...
public class StringDemo1 {
	...
	public static void main(String[] args) {
		...
		// String replace(CharSequence old, CharSequence new): 문자열에서 지정된 문자열을 새문자열로 변환한 새로운 문자열을 반환한다.
		// String replaceAll(String regexp. String replacement): 문자열에서 지정된 정규표현식과 일치하는 문자열을 찾아서 새 문자열로 변환한 문자열을 반환한다.
		String str4 = "중앙HTA학원과 새중앙HTA학원은 별도의 볍인이다.";
		String s5 = str4.replace("중앙", "현대");
		System.out.println("원본 문자열: " + str4);
		System.out.println("중앙이 현대로 바뀐 새문자열: " + s5);
		
		String str5 = "010-1111-2222, 010-2222-3333";
		String s6 = str5.replaceAll("\\d{3}", "***");
		System.out.println("숫자를 *로 바꾼 새문자열: " + s6);
		
		// String[] split(String regexp) : 문자열을 지정된 표현식(혹은 구분문자)으로 자른 새로운 배열을 반환한다.
		String str6 = "S0001,전자공학과,회로이론실습,홍길동,3";
		...
		// int lastIndexOf(String str) :   지정된 문자열이 마지막으로 등장하는 위치를 반환한다. 없으면 -1을 반환단다.
		String str7 = "홍길동은 오늘 병원에 갔다. 병원에서 주사를 맞았다.";
		boolean b3 = str7.contains("주사");
		int pos1 = str7.indexOf("병원");
		int pos2 = str7.lastIndexOf("병원");
		...
		String str8 = "                안녕        나는        로봇이야      ";
		String s7 = str8.trim();
		System.out.println("공백제거 전 [" + str8 + "]");
