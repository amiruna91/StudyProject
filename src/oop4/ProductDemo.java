		...
		while (true) {
			...
			if (menuNo == 1) {
				...
				if (stockMenuNo == 1) {
					...
					Product product = new Product(name, maker, category, price, amount);
					...
					Product product = new Product(name, maker, category, price, discount, amount);
					service.insertProduct(product);
				...
				if (searchMenuNo == 1) {
					...
					// 최소가격과 최대가격을 입력 받아서 그 가격범위내의 상품을 검색하시오.
					// TODO 서비스의 가격검색기능을 실행하시오.
					System.out.print("최소가격을 입력하세요:");
					int minPrice = scanner.nextInt();
					System.out.print("최대을 입력하세요:");
