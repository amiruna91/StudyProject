...
public class Factory {
	...
	private void createBean() {
		try {
			...
			String outputerClassName = prop.getProperty("outputer.className");
			System.out.println("리포트클래스명: " + reporterClassName);
			System.out.println("아웃풋클래스명: " + outputerClassName);

			System.out.println("객체를 생성합니다.");
			Reporter reporter = (Reporter) Class.forName(reporterClassName).newInstance();
			Output output = (Output) Class.forName(outputerClassName).newInstance();
